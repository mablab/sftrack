...
**time** - a vector containing the time information, must be either POSIX or an integer where length(time) == nrow(data). Using this argument will name the time column as 'reloc_time'. Or the column name for the time column. The output object will be sorted by the time column.
...
A common issue with movement data is when duplicated gps time stamps are logged.
When sftrack comes into contact with these records, it returns an error:
