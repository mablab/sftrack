...
crs = '+init=epsg:4326'
#bursts
burst = list(id = raccoon$sensor_code,month = as.POSIXlt(raccoon$utc_date)$mon+1)
active_burst = c('id','month')
#time
time = as.POSIXct(raccoon$acquisition_time, tz='EST')
#error
error = raccoon$fix
...
### raccoon.frame inputs  

Data.frame inputs generally describe the columns in the raccoon that represent each field. If the columns are not found in the data, an error is thrown. 
...
ltraj_df <- as.ltraj(xy=raccoon[,c('longitude','latitude')], date = as.POSIXct(raccoon$acquisition_time),
