...
`sftraj`s work nearly the same as `sftrack`s, however because they are a step model where the steps are modeled as step1 ($t_1 → t_2$) its important to note that subsetting will not automatically recalculate any new steps for you even if the original $t_2$ point has been deleted.
