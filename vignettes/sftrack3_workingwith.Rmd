...
##### Burst
The burst column is very specialized, and we will cover it in it's own section. At present, the novel attributes it stores are the 'active_burst' and the 'sort_index' which is a factor to help grouping calculations. A burst column is a class `multi_burst` consisting of multiple `ind_burst`s. `multi_bursts` are a collection of `ind_bursts` with the same `names()`. This mimics `sf` functionality where the `sfc` class is at the column level and `sfg` class is at the individual row level.

```{r}
attributes(my_sftrack$burst[1:10])
summary(my_sftrack)
```
...
An sftrack object acts like a data.frame and sf whenever appropriate. Because of this you can subset an sftrack object as you would a data.frame. Except, like sf, it attempts to retain the geometry, burst, and time columns, in order to maintain sftrack status.
