...
#### sftraj object
An sftraj object is a data.frame where the geometries are stored as a `LINESTRING` for each row. The linestring is a line built from the two points at t1 -> t2. Internally any time active_groups are changed, then the geometry is recalculated.
...
likely need to be recalculated. Internally, anytime the active group
