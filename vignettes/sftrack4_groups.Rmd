...
output:
  html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{4. Fantastic Groupss and how to use them}
...
C_groupings are a collection of s_groups with the same grouping names and one 'active_group' activated for all s_groups.
...
summary(cg_combine)
```
You can also edit groupings like a list, but you must replace it with an object of the appropriate class and active_group
```{r}
cg[1]
...
And the group names must match the ones in the c_grouping
```{r}
