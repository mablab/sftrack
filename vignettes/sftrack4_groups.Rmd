...
To begin an `s_group` is a singular grouping. It's whats stored at the row level. 
...
```{r}
library("sftrack")
group_list <- list(id = raccoons$animal_id, month = as.POSIXlt(raccoons$timestamp)$mon + 1)
...
A grouping contains group related information. The `id` of the subject/sensor is the lowest level of grouping for the data. Additional grouping variables are optional. 
...
An `s_group` is the grouping variables for a single row of data. You can make an `s_group` object using `make_s_group()`, and giving it a list with the group variables named. In this example we have a single sensor named 'TTP_058' from a raccoon, and an additional grouping variable of month (entered as its numeric interpretation).
...
`c_grouping`s are a collection of s_groups with the same grouping names and an 'active_group' which is a subset of all the available group names.
...
As `c_groupings` are stored as lists, it can be difficult to refer to
a single group or combination of groupings. This is where the
`sort_index` can come in handy. The `sort_index` is a factor of the
combined active_group variables for each row where 
`paste(id, name1, name2, ..., sep = "_")`. The sort index is remade every time the active group changes, and therefore can be used to subset. You can access the sort index using `group_labels()`. 
...
## Selecting group "TTP-058_2"
print(racc_traj[group_labels(racc_traj) %in% c("TTP-058_2"), ], 6)
...
The active group is a simple yet powerful feature. It dictates how your data is grouped for essentially all calculations. It can also be changed on the fly. You can view and change the active group of a column grouping with `active_group()`. Once changed, it recalculates the `sort_index` and in some cases recalculates the geometries.

Active groups can be changed for any `sftrack`/`straj`/`c_grouping`.

```{r}
## sftrack/sftraj
active_group(racc_traj)
summary(racc_traj, stats = TRUE)

## Change the active group to id only
active_group(racc_traj) <- c("id")
active_group(racc_traj)
summary(racc_traj, stats = TRUE)
