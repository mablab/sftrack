...
variables is the track or trajectory column, which will store tracking
data (tracks) and movement data (trajectories), respectively
...
Two new classes of `data.frame`s will be defined to store <!-- ` -->
tracking data (`c("sftrack", "sf", "data.frame")`) or movement data
(`c("sftraj", "sf", "data.frame")`).  Correspondingly, the step column
(list-column) will be of class `sfc_TRACK` or `sfc_TRAJECTORY`, with a
unique Coordinate Reference System (CRS) and timezone in the two
(constant) attributes `crs` and `timezone`, respectively. The
implementation is further complicated by the fact that a full
correspondence between a *track* and a *trajectory* needs to be
achieved, while there is no bidirectional navigation between a
