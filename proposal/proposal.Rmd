...
flexible approach based on simple `data.frame`s using `sf` spatial
building blocks, as a cornerstone for the development of a more
cohesive movement community in R. The only aim of the package will be
to present this central class and basic functions to build, handle,
...
objects (Turchin 1998), which can be decomposed as a series of
straight-line segments called *moves* between stopping points where
the object turns. Moves can be of varying length and associated time
span, depending on the nature of the movement. Treating moves as
straight lines is an "idealization", but at sufficient resolution, no
significant information is lost.

<!-- Turchin p.129 -->

In practice, this curve is sampled (recorded) at discrete times with
4D vertices of coordinates (3-dimensional spatial coordinates and
temporal coordinate), which define *tracking data* of the form $(x, y,
z, t)$, or, according to Turchin (1998), a "path [i.e.] the complete
spatio-temporal record of a followed organism, from the beginning to
the end of observations".

<!-- Is that the opposite? I.e. path in real world; trajectory sampled? -->

Turchin then defines *steps* as the straight-line segments formed by
successive vertices at regular sampling interval. Let us generalized
to all time intervals, as sampling is not necessarily at regular
intervals. In most cases, a step does not correspond to a move.  A
trajectory is thus recorded by a series of points (tracking data),
which forms steps, and ultimately a path. It is important to recognize
this dual perspective, between tracking data (points) and movement
data (steps).
...
### Proposed data model of a trajectory
...
real world entities (here a trajectory). Following the approach of the
ANSI, we can define three kinds of data models: conceptual, logical,
and physical. The conceptual data model is the highest-level model,
and describes the semantics of the model and its scope; the logical
data model translates this into general data concepts; the physical
data model implements it in coding terms and solutions (here in R).

```{r data-model, echo = FALSE, out.width = "100%", fig.cap = "Proposed data model.", fig.align = "center"}
knitr::include_graphics("figures/data-model.png")
```
...
given times with 4-dimensional vertices of coordinates $(x, y [, z],
t)$, which define a path made of sequential steps, i.e. the
...
logical data model (Fig.1B). Data are provided as tidy data, in a data
