...
#' my_track <- as_sftraj(raccoon, burst = burstz, time = 'acquisition_time',
#'   error = NA, coords = c('longitude','latitude'))
#'
#'   # Input is a ltraj
#'   library(adehabitatLT)
#'   ltraj_df <- as.ltraj(xy=raccoon[,c('longitude','latitude')],
#'   date = as.POSIXct(raccoon$acquisition_time),
#'   id = raccoon$sensor_code, typeII = TRUE,
...
as_sftraj.sf <-  function(data,
  ...
  error = NA) {
  # data(raccoon)
  # burst = list(id = raccoon$sensor_code,month = as.POSIXlt(raccoon$utc_date)$mon, height =as.numeric(raccoon$height>5))
...
rbind.sftraj <- function(...) {
  ...
  same_att <-
    all(duplicated(lapply(all, function(x)
      attributes(x)[c('sf_column', 'time', 'error')]))[-1])
  if (!same_att) {
  ...
  step_geometry <-
    make_step_geom(burst = df1$burst,
      geometry = geom,
