...
#'   group = group, time = "timestamp",
#'   error = NA, coords = c("longitude", "latitude")
#' )
#'
#' # Input is a sf object
#' library("sf")
#' df1 <- raccoons[!is.na(raccoons$latitude), ]
...
as_sftrack.sftraj <- function(data, ..., fill_missing = FALSE) {
  ...
  data <- create_missing_traj(data, fill_missing = fill_missing)
...
#' @export
print.sftrack <- function(x, n = getOption("sf_max_print", default = 6), n_col, ...) {
  if (missing(n_col)) {
    n_col <- ncol(x)
  }
