test_that("checks check successfully", {
  ...
  expect_error( as_sftrack(data = df1,burst=list(id=df1$id, month = df1$month),
    ...
    expect_message(as_sftrack(data = df1,burst=list(id=df1$id, month = df1$month),
    ...
    expect_error(as_sftrack(data = df2, burst = c('id','month'),
    ...
    expect_message(as_sftrack(data = df4,burst = c('id','month'),
      time = 'timez', coords = c('x','y')),
