test_that("sftraj is built correct", {
  ...
  my_sftraj <- suppressMessages(as_sftraj(data = df1,burst=list(id=df1$id, month = df1$month),
    time = 'timez', active_burst = c('id'), coords = c('x','y','z')))
  ...
  my_sftraj_2d <- suppressMessages(as_sftraj(data = df1,burst=list(id=df1$id, month = df1$month),
    time = 'timez', active_burst = c('id'), coords = c('x','y')))
