test_that("step geometry calculates correctly", {
  ...
  my_track <- as_sftrack(data = df1,burst=list(id=df1$id), time = 'timez', coords = c('x','y','z'))

  my_step_geom <- make_step_geom(burst_id = my_track$burst, geometry = my_track$geometry, time_data =my_track$timez)
  ...
  my_track <- as_sftrack(data = df1,burst=list(id=df1$id, month = df1$month), time = 'timez', coords = c('x','y','z'))
